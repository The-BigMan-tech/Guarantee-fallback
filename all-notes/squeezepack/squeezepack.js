    ...
    while ((leftChunks.at(-1).length > 16)) { //Breaks the first chunk to smaller chunks
        ...
        if (lastLeftChunk.endsWith('0')) { //This is to ensure that the integer zero cant be its own chunk because its used by my algorithm as a terminator or the center between the left and right chunks of data in the compressed array
            ...
            for (let i = (lastLeftChunk.length); i >= 0; i--) {
                const zeroOrNonzero = Number(lastLeftChunk[i]);
                if (zeroOrNonzero > 0) {
            ...
            const thirdToLastLeftChunk = leftChunks[leftChunks.length - 3] || lastLeftChunk;
...
function retrieveElement(array, index) {
    return DecodeForIndex(Dechunk(array), index);
}
function getSize(array) {
    return (array.length * 8);
}
const arrA = [1, 2, 3, 4, 4];
console.log('Uncompressed Array Size in bytes: ', getSize(arrA));
const compArray = Compress([1, 2, 3, 4, 45]);
