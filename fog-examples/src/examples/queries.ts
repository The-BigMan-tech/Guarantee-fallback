...
import { checkBy, importDocFromObject, Result } from "fog-js";
...
const answer1 = await doc.isItStated(checkBy.Membership,'friends',["Billy","Mark"]);
doc.printAnswer(answer1);

const answer2 = await doc.isItImplied('brothers',["Matt","Philip"]);//it will return an error if the query statement broke the rule's validation schema
