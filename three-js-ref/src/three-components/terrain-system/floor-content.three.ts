...
export class FloorContent {
    ...
    constructor(floorContentData:FloorContentData,chunkPos:THREE.Vector3) {
        ...
        const minHeight = 2.1;//im not exposing these as part of the interface cuz the cubes are just placeholders for content like trees,structures but not blocks for terrain cuz that one will use a different algorithm for placement.so the interface shouldnt be tied to speciic content till i make sub or behaviour classes
        const maxHeight = 2.1;
        const width = 20;
