...
list: LSQUARE ((primitive | ref | list) (COMMA (primitive | ref | list))*)? RSQUARE;
...
ALIAS: HASH PLAIN_WORD;

NAME: (COLON PLAIN_WORD) | (EXCLAMATION PLAIN_WORD);//
NUMBER: '-'? DIGIT+ ('.' DIGIT+)?;//
PLAIN_WORD: LETTER (LETTER | DIGIT | '_')*;

primitive:NAME | NUMBER;
ref: GENERIC_REF | SINGLE_SUBJECT_REF | GROUP_SUBJECT_REF | SINGLE_OBJECT_REF | GROUP_OBJECT_REF;
