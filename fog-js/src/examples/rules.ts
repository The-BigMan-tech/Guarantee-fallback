...
export const rules = {//A rule is a function that takes a document and a statement and tells if that statement is true from the given facts in the document whether it was explicitly stated or by inference from the rule itself.
    directFriends:async (doc:Doc<P>,statement:string[])=> {
        return doc.isItStated('friends',statement,Check.byMembership);//its checking by membership not strict order to handle a statement of variable args and also when the order requirement isnt strict
    },
    indirectFriends:async (doc:Doc<P>,statement:[string,string],visitedCombinations:string[])=> {
        ...
        const {candidates,checkedCombinations} = await doc.genCandidates(1,'friends',statement,visitedCombinations) as GeneratedCandidates<string>;
    ...
    brothers:async (doc:Doc<P>,statement:[string,string]) => {
        const [X,Y] = statement;
        if (X === Y) return false;
        const isXMale = await doc.isItStated('male',[X],Check.byMembership);
        const isYMale = await doc.isItStated('male',[Y],Check.byMembership);
